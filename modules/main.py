...
async def start(client: Client, msg: Message):
    ...
    await start_message.edit_text(
        Data.START.format(msg.from_user.mention) +
        "CHECKING OK... Command Nhi Bataunga **•.•.—(••÷[ 𝗕𝐨ƚṂꪋ𝚍𝑒𝗕𝒀 ]÷••)—.•.• ᯓ𐌑ⲅ.𝙱𝚎𝚛𝚕𝚒𝚗↭ ♚™👨🏻‍💻**🔍\n\n"
        "Progress:[🟩🟩🟩🟩🟩🟩🟩🟩🟩] 100%\n\n"
    )
...
async def txt_handler(bot: Client, m: Message):
    ...
    await editable.edit("**send '1' for use default**")
...
@bot.on_message(filters.command(["berlin1"]) )
async def txt_handler(bot: Client, m: Message):
    ...
    try:
        for i in range(arg-1, len(links)):
            ...
            try:  
                ...
                cc = f'**🎞️ VID_ID: {str(count).zfill(3)}.\n\nTitle: {name1} ᯓ𐌑ⲅ.𝙱𝚎𝚛𝚕𝚒𝚗↭ ♚ {res}.mkv\n\n📚 Batch Name: {b_name}\n\n📥 Extracted By : {CR}\n\n**━━━━━✦ᯓ𐌑ⲅ.𝙱𝚎𝚛𝚕𝚒𝚗↭ ♚✦━━━━━**'
                cc1 = f'**📁 PDF_ID: {str(count).zfill(3)}.\n\nTitle: {name1} ᯓ𐌑ⲅ.𝙱𝚎𝚛𝚕𝚒𝚗↭ ♚.pdf\n\n📚 Batch Name: {b_name}\n\n📥 Extracted By : {CR}\n\n**━━━━━✦ᯓ𐌑ⲅ.𝙱𝚎𝚛𝚕𝚒𝚗↭ ♚✦━━━━━**'
...
@bot.on_message(filters.command(["berlin2"]))
async def txt_handler(bot: Client, m: Message):
    editable = await m.reply_text(f"**🔹Send me the TXT file and wait.**")
    input: Message = await bot.listen(editable.chat.id)
    x = await input.download()
...
@bot.on_message(filters.command(["berlin3"]))
async def txt_handler(bot: Client, m: Message):
    editable = await m.reply_text(f"**🔹Send me the TXT file and wait.**")
    input: Message = await bot.listen(editable.chat.id)
    x = await input.download()
